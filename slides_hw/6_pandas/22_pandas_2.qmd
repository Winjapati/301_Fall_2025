...
``` python
lang = [key for key in languages.keys()] fam = [value for value in languages.values()] print(lang, fam)
```

## Activity

``` python
import pandas as pd  langs = pd.Series(lang, name="languages") fams = pd.Series(fam, name="families") langs, fams
```
...
``` python
df.head() df.tail(2) df.shape, df.columns, df.index df.info()
```
