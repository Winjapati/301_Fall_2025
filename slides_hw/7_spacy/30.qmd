...
- Copy the following list and sentence.  Then print tokens & lemmas of those words found in the sentence that are direct objects or objects of prepositions.
...
# Step 2: Sample text
text = "The farmer owned three dogs: a beagle, a shepherd, and a terrier, but the poodle ran away with a collie."

# Step 3: Tokenize and lemmatize only words that match our semantic group
doc = nlp(text)
...
for tok in doc:
    if tok.pos_ == "VERB":
        dobj = [c for c in tok.children if c.dep_ == "dobj"]
        if dobj:
            objects.append(dobj[0].text)
    if tok.pos_ == "ADP":
        pobj = [c for c in tok.children if c.dep_ == "dobj"]
        if dobj:
            objects.append(pobj[0].text)
